...
def preProcessing(A, normFac, EVT=None):
    ...
    inputs_cat0 = A[:, :, 8]  # encoded PF pdgId
    inputs_cat1 = A[:, :, 7]  # encoded PF charge

    return inputs, pxpy, inputs_cat0, inputs_cat1


def preProcessingForTinyTableModel(A, normFac, EVT=None):
    """ pre-processing input """
    ...
    # eta_idx[(abs_eta >= 0.0) & (abs_eta < 1.3)] = 0
    # eta_idx[(abs_eta >= 1.3) & (abs_eta < 1.7)] = 1
    # eta_idx[(abs_eta >= 1.7) & (abs_eta < 2.5)] = 2
    # eta_idx[(abs_eta >= 2.5)] = 3

    abs_eta = np.abs(eta)
    # etaphi_lsb = np.pi / 720
    # eta_idx = np.rint(abs_eta / etaphi_lsb).astype(np.int32)
    eta_idx = np.zeros_like(abs_eta, dtype=np.int32)
    eta_idx[(abs_eta >= 0.0) & (abs_eta < 1.0)] = 0
    eta_idx[(abs_eta >= 1.0) & (abs_eta < 1.3)] = 1
    eta_idx[(abs_eta >= 1.3) & (abs_eta < 1.5)] = 2
    eta_idx[(abs_eta >= 1.5) & (abs_eta < 1.7)] = 3
    eta_idx[(abs_eta >= 1.7) & (abs_eta < 2.5)] = 4
    eta_idx[(abs_eta >= 2.5) & (abs_eta < 3.0)] = 5
    eta_idx[(abs_eta >= 3.0)] = 6

    # pT: ap_fixed<14, 12> 0 GeV to 2047 GeV - maximum 256 bins
    # 0 - 256      2 GeV gap - 128 bin
    # 256 - 2048  14 GeV gap - 128 bin
    pT_idx = np.zeros_like(pt, dtype=np.int32)
    low_pT = np.arange(0, 256, 2)
    high_pT = np.arange(256, 2048, 14)
    edges = np.concatenate((low_pT, high_pT, [2048]))

    for i in range(256):
        pT_idx[(pt >= edges[i]) & (pt < edges[i + 1])] = i

    # overflow protection
    pT_idx[pt >= 2048] = 255
    pT_idx[pt < 0] = 0
    # pT_idx = (pt * 4.0).astype(np.int32)
    

    inputs_pID = A[:, :, 6:7]
    features = np.concatenate((inputs_pID, eta_idx, pT_idx), axis=2)
    inputs_pxpy = np.concatenate((px, py), axis=2)

    return features, inputs_pxpy
...
#     XYbinnings = np.linspace(10, 400, num=nbins+1)  # create 20 bins for pt from 0 to 400 GeV
#     truthX_means, binX_edges, binXnumber = binned_statistic(np.abs(trueXY[:, 0]), np.abs(trueXY[:, 0]), statistic='mean', bins=XYbinnings, range=(10, 400))
#     mlX_means, _, _ = binned_statistic(np.abs(trueXY[:, 0]), np.abs(mlXY[:, 0]),
#                                        statistic='mean', bins=XYbinnings, range=(10, 400))
#     puppiX_means, _, _ = binned_statistic(np.abs(trueXY[:, 0]), np.abs(puppiXY[:, 0]),
#                                          statistic='mean', bins=XYbinnings, range=(10, 400))

#     truthY_means, binY_edges, binYnumber = binned_statistic(np.abs(trueXY[:, 1]), np.abs(trueXY[:, 1]), statistic='mean', bins=XYbinnings, range=(10, 400))
#     mlY_means, _, _ = binned_statistic(np.abs(trueXY[:, 1]), np.abs(mlXY[:, 1]),
#                                        statistic='mean', bins=XYbinnings, range=(10, 400))
#     puppiY_means, _, _ = binned_statistic(np.abs(trueXY[:, 1]), np.abs(puppiXY[:, 1]),
#                                          statistic='mean', bins=XYbinnings, range=(10, 400))
...
#     # plot response
#     plt.figure()
#     plt.hlines(truthX_means/truthX_means, binX_edges[:-1], binX_edges[1:], colors='k', lw=5,
#                label='Truth', linestyles='solid')
#     plt.hlines(mlX_means/truthX_means, binX_edges[:-1], binX_edges[1:], colors='r', lw=5,
#                label='ML', linestyles='solid')
#     plt.hlines(puppiX_means/truthX_means, binX_edges[:-1], binX_edges[1:], colors='g', lw=5,
#                label='PUPPI', linestyles='solid')
#     plt.xlim(10, 400.0)
#     plt.ylim(0, 1.1)
#     plt.xlabel('Truth MET [GeV]')
#     plt.legend(loc='lower right')
#     plt.ylabel('<MET Estimation>/<MET Truth>')
#     plt.savefig(f"{path_out}METX_response.png")
#     plt.close()

#     # plot response
#     plt.figure()
#     plt.hlines(truthY_means/truthY_means, binY_edges[:-1], binY_edges[1:], colors='k', lw=5,
#                label='Truth', linestyles='solid')
#     plt.hlines(mlY_means/truthY_means, binY_edges[:-1], binY_edges[1:], colors='r', lw=5,
#                label='ML', linestyles='solid')
#     plt.hlines(puppiY_means/truthY_means, binY_edges[:-1], binY_edges[1:], colors='g', lw=5,
#                label='PUPPI', linestyles='solid')
#     plt.xlim(10, 400.0)
#     plt.ylim(0, 1.1)
#     plt.xlabel('Truth MET [GeV]')
#     plt.legend(loc='lower right')
#     plt.ylabel('<MET Estimation>/<MET Truth>')
#     plt.savefig(f"{path_out}METY_response.png")
#     plt.close()
